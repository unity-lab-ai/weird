// DUNGEON MASTER: THE HUNT — escape-recovery hunt page.

(function () {
  'use strict';

  function render(el) {
    const recoverable = window.DMTHGame.escapeRecovery.recoverable();
    ...
    el.innerHTML = `
      ...
      ${recoverable.length === 0 ? `<div class="panel"><p class="muted small">No escapees to chase. Good containment.</p></div>` :
        `<div class="panel">
          <div class="girl-grid">
            ${recoverable.map(g => {
              const remainingMin = Math.round(window.DMTHGame.escapeRecovery.timeRemaining(g) / 60000);
              return `<div class="girl-card">
                ...
                ${tools.length === 0 ? `<p class="muted small">No tools. <a href="#shop">Shop →</a></p>` :
                  `<div class="btn-col">${tools.map(tid => {
                    const tool = window.DMTHAssets.getById('item', tid);
                    const boosted = { ...g, stats: { ...g.stats, defiance: Math.min(99, g.stats.defiance + 20) } };
                    const odds = window.DMTHGame.hunt.previewCaptureOdds({ girl: boosted, toolId: tid, locationId: 'street' });
    ...
    el.querySelectorAll('[data-recapture]').forEach(b => {
      b.onclick = async () => {
        ...
        try {
          const r = window.DMTHGame.escapeRecovery.attemptRecapture({ girl, toolId });
  ...
  window.DMTHRouter.register('escape-recovery', render);
})();
