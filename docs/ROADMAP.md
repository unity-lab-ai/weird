...
### Milestone 21.11 — Capture as multi-stage progress-bar mechanic (REFORMULATED 2026-05-14)
...
- [ ] T36.30 — **Capture stage engine** — new module `js/game/capture.js` exposing `SSDGame.capture`. 4-stage state machine: `Approach`, `Engage`, `Subdue`, `Secure`. Each stage holds a 0-100% progress meter. Per-stage resolution math: `stageProgress += (toolStageBonus + playerSkill - girlStageResistance - locationDifficulty - witnessPenalty)` per action. Stage clears when meter hits 100; full capture success only when all 4 clear. Any stage hitting 0 = attempt fails, girl escapes, witness roll fires, location cooldown bumps.
- [ ] T36.31 — **Per-tool stage profile in catalog** — every capture tool in `js/assets/catalog.js` gains a `captureStages: { approach: 0-50, engage: 0-50, subdue: 0-50, secure: 0-50 }` stat block. Examples: `pipe` = `{approach: 10, engage: 0, subdue: 25, secure: 0}` (stealth + blunt-force subdue); `rohypnol` = `{approach: 0, engage: 30, subdue: 15, secure: 0}` (social engagement + slow incapacitation); `chloroform` = `{approach: 0, engage: 25, subdue: 35, secure: 0}` (fast engage + heavy subdue); `duct-tape` = `{approach: 0, engage: 0, subdue: 0, secure: 30}` (binding only); `zip-ties` = secure 25; `handcuffs` = secure 40; `ether` = engage 40, subdue 30; `ketamine` = subdue 50. Tool selection per stage becomes meaningful.
- [ ] T36.32 — **Per-archetype stage resistance** — each girl-archetype in `js/templates/archetypes/` gets a `captureResistance: { approach, engage, subdue, secure }` weights object. Library: low across the board. Street: high subdue (fights dirty). Gym: very high subdue (physical). Sorority: high engage (alerts others). Club: high approach (crowded environment). Barista: low across the board. Rolled into `girl.captureResistance` at girl-gen so each captive has the archetype's profile.
- [ ] T36.33 — **Progress-bar UI in `js/ui/hunt.js`** — 4 stacked 0-100% progress bars (one per stage), with current stage highlighted, tool-loadout slots above each bar (drag/click-to-assign a tool from inventory), real-time fill animation as each action resolves. Show resistance markers on each bar (e.g. red zone where archetype resistance overcomes typical tool progress so the player sees they need a stronger tool there).
- [ ] T36.34 — **Multi-tool attempt sequencing + per-stage inventory consumption** — the user picks one tool per stage before initiating the attempt. Single-use items (chloroform rag, rohypnol vial, ether bottle, ketamine dose, duct tape strip) are consumed PER STAGE THEY'RE ACTIVATED IN — not per attempt. Multi-use items (pipe, handcuffs) are reusable across stages within an attempt. Inventory validation runs at stage-start: if the tool slotted isn't available, that stage stalls at 0% and resistance overwhelms it.
- [ ] T36.35 — **Outcome resolver hooks** — at Stage 4 (Secure) completion: success path triggers the existing capture-flow narrative (depends on PRE.2 4-beat transition: subdue → transport → arrival → first conscious moment). Failure path (any stage hits 0): girl escapes, location notoriety bumps, witness pool rolls (witnesses → suspicion spike), per-tool location cooldown applies (used rohypnol today? Engage tool selection limited at this location for 30 in-game minutes), and the girl gains a `wariness` flag making her next encounter harder.
...
8. Phase 21.11 (Capture as multi-stage progress-bar mechanic, ~3-4h) — capture becomes a 4-stage Approach → Engage → Subdue → Secure attempt sequence with per-tool stage stats and per-archetype stage resistance. Depends on existing Tool × Woman × Location scene-template + Full capture transition epics from pre-2026-05-14 backlog.
...
| 2026-05-14 | **Capture as multi-stage progress-bar mechanic** (REFORMULATED 2026-05-14) — capture becomes a 4-stage attempt sequence (Approach → Engage → Subdue → Secure), each stage with its own 0-100% progress meter driven by the selected tool's per-stage stats vs the girl-archetype's per-stage resistance. Tools become stage-specific (rohypnol → Engage; duct tape → Secure; chloroform → Engage+Subdue; pipe → Approach+Subdue; ketamine → Subdue heavy). Stage clears at 100%; full success only when all 4 clear; any stage hitting 0 = attempt fails. Spam dies as a play pattern because mashing one tool advances ONE meter — the other three stages still need their own qualifying tool. Original anti-spam friction (witness roll, location cooldown, girl-wariness) is collapsed into the failure-path consequences of the new mechanic. | Gee verbatim 2026-05-14: *"the capture girls part needs worked out better currntly i jsut spam items until their caught"* + reformulation *"phase 21.11 isnt exactly right its just that the capture a girl process needs to have like progress bar with true mechanics to it not just something random thats not truew to the tools and options said think about it and how u need to reformulate this task"*. |
