...
### Milestone 21.20 — Films auto-sell + sell-negatives premium action (Gee verbatim 2026-05-14: *"lest also get rid of the slaes pass button for sales of videos and just have them auto sell and u never lose a video as u can make many copies so they are always for sale it jsut u can remove them(sell negatives) which gives much more $ than the noraml video sales that are more like passive income"*)

Films become passive-income assets. The "sales pass" button goes away — films auto-sell each market tick at small per-tick rates without ever being consumed (infinite-copies model). New "💣 Sell negatives" action per film destroys the master / removes the film from circulation in exchange for a one-time premium payout (3-5× total expected lifetime passive). Films page surfaces passive earnings ticker per film + the big-payout button per film.

- [ ] T36.82 — Delete the "Sales pass" button trigger from `js/ui/market-view.js`. The button's logic (calling `SSDGame.market.runSaleTick()`) moves entirely into the background `tick.js` schedule. UI surfaces "🔄 Auto-selling on tick" status text instead of the click-to-run button.
- [ ] T36.83 — Rewrite `SSDGame.market.runSaleTick()` semantics — sale "consumes" the film NO LONGER. Each tick, every listed film generates `payout = basePrice × tickRate` (tickRate small, 0.02-0.05 per tick depending on demand). Film stays in catalog forever as a passive earner.
- [ ] T36.84 — Add `sellNegatives(filmId)` on `SSDGame.market`. Computes `premiumPayout = basePrice × 3.5 × demandMultiplier`. Removes the film from `films` array. Logs a one-shot event ("Negatives sold — $X").
- [ ] T36.85 — Films UI in `js/ui/market-view.js` shows per-film passive-earnings ticker (live updating from state) + big "💣 Sell negatives — $X" button per film with confirm-dialog ("destroys master, removes from catalog — proceed?").
- [ ] T36.86 — Backwards-compat: existing saves' film records continue to work. `runSaleTick()` reads the new tickRate field with safe default if missing.

### Milestone 21.21 — Disposal method final-image generation per method (Gee verbatim 2026-05-14: *"and the dispose option needs to show like the image of the grave, the water, the crematoryei burning, ect ect for each one the final thing is the image of it"*)

After each disposal method's Ollama narration completes, generate + display a final-scene Pollinations image showing the actual disposal scene. The image is the permanent record — captures the moment in image form alongside the FINALIZED narration.

- [ ] T36.87 — Per-method final-scene image prompts in `js/game/imaging.js` (or new helper module). Method → scene prompt map:
  - **bury** → "freshly dug grave mound of dark soil in a wooded clearing, shovel resting against a tree, last-light dusk, documentary photography"
  - **drown / water-disposal** → "weighted body sinking into dark water seen from beneath the surface, fading silhouette descending, deep-water blue-black, fine-art photography"
  - **cremate** → "industrial crematory furnace door open with flames inside, scattered ash on a steel tray, dim industrial lighting, documentary photography"
  - **release** → "young adult woman walking away down a dawn country road, back to camera, full body, returning to the world, soft golden-hour light, editorial photography"
  - **finalization-film** → "editorial film-poster frame of an adult woman, dramatic low-key lighting, cinematic, professional poster framing"
- [ ] T36.88 — `generateDisposalFinalImage({method, girl})` composes the prompt with the girl's locked face + seed (so she's recognizable in the scene, where applicable) + method-specific environment + final-state body markers. Calls `generateFor()` with a custom prompt override path.
- [ ] T36.89 — `js/ui/dispose-view.js` updated — after Ollama narration completes for the chosen method, render the generated image as the final disposal screen below the narration. Lazy-load with fallback to text+emoji if Pollinations unavailable.
- [ ] T36.90 — Cache the disposal image per `(girlId × method)` in IDB so it's a permanent record (visible later in FINALIZED-disposal entries / disposal log).
- [ ] T36.91 — Existing image-pipeline guarantees enforced: adult-floor age via `${girl.age}` (Phase 21.1), full-body framing (Phase 21.15 + 21.3), `sanitizePrompt` fallback on 403 (existing).
...
| 2026-05-14 | **Films auto-sell + sell-negatives premium action** (Phase 21.20) — kills the "sales pass" button. Films auto-sell on `tick.js` schedule at small per-tick passive rates without ever being consumed (infinite-copies). New `sellNegatives(filmId)` action destroys the master / removes from catalog for a one-time premium payout (3.5× × demand multiplier). Films market UI surfaces per-film passive-earnings ticker + big "💣 Sell negatives" button per film with confirm-dialog. Existing saves backward-compatible (tickRate field optional). | Gee verbatim 2026-05-14: *"lest also get rid of the slaes pass button for sales of videos and just have them auto sell and u never lose a video as u can make many copies so they are always for sale it jsut u can remove them(sell negatives) which gives much more $ than the noraml video sales that are more like passive income"*. |
| 2026-05-14 | **Disposal method final-image generation** (Phase 21.21) — after each disposal method's Ollama narration completes, render a final-scene Pollinations image showing the actual disposal scene. Bury → grave mound + shovel. Drown → weighted-body sinking. Cremate → furnace flames + ash. Release → her walking away to dawn road. Finalization → editorial poster frame. Composes with girl's locked face + seed (recognizable where applicable) + method environment + final-state body. Cached per `(girlId × method)` so each disposal becomes a permanent IDB record. All existing image-pipeline guarantees (adult-floor age, full-body framing, sanitize) honored. | Gee verbatim 2026-05-14: *"and the dispose option needs to show like the image of the grave, the water, the crematoryei burning, ect ect for each one the final thing is the image of it"*. |
