...
- [ ] **T36.105** 🔴 **Girl stats drive john happiness → john pay multiplier** (Gee verbatim 2026-05-14, mid-ship addendum during Phase 21.17 work: *"better girls gorwen stats = hap[pier johns= more money"*) — Phase 21.16 john-resolver's payment calculation must scale with the girl's stats. Inputs: Stockholm bond level (higher = trained = bigger payout), stamina + health (above threshold = good service = bigger payout, below threshold = poor service = john leaves dissatisfied + smaller payout), mood (good mood = happy john), arousal/wetness/cumLoad envelope, outfit tier multiplier, kink-match score against john's preferences, repeat-client reputation. Formula sketch: `pay = basePay × (1 + bondLevel*0.10) × staminaFactor × healthFactor × moodFactor × outfitMultiplier × kinkMatchScore`. johnHappiness >= 0.8 → +10% tip chance; <= 0.3 → john leaves bad rep + -2 notoriety event. Cross-link with Phase 21.17 (stamina/health are the body-stats driving this) + Phase 21.10 (pregnancy state may suppress / boost happiness depending on john archetype's pregnant-want preference).
